  ...
  return (
    <div className="max-w-5xl mx-auto py-12 md:py-16 px-5 md:px-8 w-full animate-fade-in">
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 mb-8 pb-4 border-b-2 border-rule">
        <div>
          <h1 className="text-4xl font-display mb-1">Exclusion Registry</h1>
          <p className="text-sm text-ink/60 font-mono">{data.length} CANDIDATES FLAGGED AS HONEYPOTS</p>
        </div>
      ...
      {flagStats.length > 0 && (
        <div className="mb-8">
          <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-ink/50 mb-3 flex items-center gap-1.5">
            <Flag size={12} /> Flag Distribution
          </h3>
          <div className="flex flex-wrap gap-2">
            {flagStats.map(([flag, count]) => (
              <div key={flag} className="bg-card border border-caution/20 px-3 py-2 text-xs font-mono flex items-center gap-2 card-hover">
                <span className="text-caution font-bold">{count}</span>
                <span className="text-ink/70">{flag.replace(/_/g, ' ')}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {data.length === 0 ? (
        <div className="p-12 text-center bg-card border-2 border-rule shadow-sm flex flex-col items-center gap-3 card-hover">
        ...
        <div className="p-12 text-center bg-card border-2 border-rule shadow-sm">
          <span className="font-mono text-sm text-ink/50">No results match your search.</span>
        </div>
      ) : (
        <div className="grid gap-5 md:grid-cols-2">
          {filtered.map((h, i) => (
            <div
              ...
              className="bg-card border-2 border-rule p-6 shadow-sm flex flex-col justify-between card-hover animate-fade-in-fast"
              style={{ animationDelay: `${i * 50}ms` }}
            >
              <div>
                <div className="font-mono text-[10px] text-ink/40 mb-1 uppercase tracking-wider">Candidate ID</div>
                <h2 className="font-mono text-lg font-bold mb-4 text-ink">{h.candidate_id}</h2>
              </div>
                ...
                <ul className="space-y-2">
                  {h.flags.map((f: string, j: number) => (
                    <li key={j} className="flex gap-3 items-start text-sm">
                      ...
                      <span className="leading-snug text-ink/90">{f.replace(/_/g, ' ')}</span>
                    </li>
                  ))}
                </ul>
              </div>
